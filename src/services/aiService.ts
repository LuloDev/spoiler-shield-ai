...
import type { ResultMovie, ResultTv } from "src/models/tmdb/tmdbSearch";
...
export class AiService {
  ...
  private getPrediction = async (content: InfoContent[]) => {
    const text = content
      ...
      .join("\n");
    const { tvShows: tv, movies: m } = await chrome.storage.sync.get([
      "tvShows",
      "movies",
    ]);
    const tvShows = tv
      ...
      .join("\n");
    const movies = m
      .map((movie: ResultMovie) => `${movie.title} (${movie.original_title})`)
      .join("\n");
    ...
    const result = await generateObject({
      ...
      system:
        "You are an AI assistant tasked with evaluating YouTube video titles for potential spoilers of specific TV shows or movies. Follow these instructions:" +
        " 1. Read the title of the video and the name of the TV show or movie." +
        " 2. You will be given a list of TV shows or movies to avoid spoilers for, and a YouTube video title." +
        ...
        '   "name": The name of the TV show or movie' +
        ...
        " 5. Definition of a spoiler: Information revealing important plot points, character deaths, plot twists, or future events of the tv show or movie." +
        " 6. In case of ambiguity, evaluate based on the title's relevance to the plot. If the title seems unrelated to the tv show or movie or its genre, assign a value close to 0. " +
        ...
        ` "Deadpool and Wolverine: All deadpools in the movie" - 0.7 (Moderate to the movies)` +
      ...
      prompt:
        ...
        `Movies to search spoilers: ${movies}\n` +
